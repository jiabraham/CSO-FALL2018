...
{	
	//declared found to test if a hash match is present
	bool found = true;

	//Iterative loop to check all the different hash values that must match
	for (int i=0; i < BLOOM_HASH_NUM; ++i) {

		//Calculates hash of new element
		int hashi = hash_i(i, elm);

		
		int bloom_pos = hashi % f->bsz;
		if (!bloom_bit_at_pos(f, bloom_pos)) {
			found = false;
			break;
		}
	}
