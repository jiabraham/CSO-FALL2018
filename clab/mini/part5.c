...
list_insert(struct list_node *head, int value)
{
	...
	// TODO: Your code here.
	//Create a new node
	struct list_node* add = alloc_node();

	//Initialize value
	add -> value = value;
	//Set next node
	add -> next = (head -> next);
	//Set new head reference
	head -> next = add;
}
...
{	
	//Check to see if head is the only element in the linked list
	if (head -> next == NULL) {
		return head;
	}
	...
	//Loop to go through the list to find the very last node
	while (temp -> next != NULL) {
		temp = temp -> next;
	}
	//Return temp because it now points to the last node
	return temp;
...
list_size(struct list_node *head)
{
	...
	// TODO: Your code here.
	//Initialize count which has been specified to always be at least 1
	int count = 1;
	//Create temporary node to cycle through the list
	struct list_node *temp = head;
	//Loop to cycle through
	while (temp -> next != NULL) {
		//Increment count every time it's not the end
		count++;
		//Increment temp to move along the list
		temp = temp -> next;
	}
	//Return count of how many nodes are contained
	return count;
}
...
list_find(struct list_node *head, int value, struct list_node **predp)
{
	...
	//Check first if head has the value in question
	if (head -> value == value) {
		*predp = NULL;
		return head;
	}
	...
	// TODO: Your code here.
	//Create temporary pointer pointing to head.next
	struct list_node *temp = head;
	//Create a temporary pointer pointing to head
	struct list_node *prev = NULL;
	//Loop to cycle through the linked list
	while (temp != NULL) {
		//Check to see if temp contains the value
		if (temp -> value == value) {
			//set predp equal to the previous node
			*predp = prev;
			return temp;
		}
	...
	//Only occurs if no such value exists
	*predp = NULL;
	return NULL;
}
...
list_remove(struct list_node **headp, int value)
{
	...
	//Create double pointer to be the address for pointer headp
	struct list_node *prev = NULL;

	//If headp contains the value 
	if ((*headp) -> value == value) {
		//head reference can be moved directly
		prev = *headp;
		*headp = (*headp)->next;
	...
	//Set new node pointer remove to the node in question
	struct list_node *remove = list_find(*headp, value, &prev);
	//Increment next field for previous pointer
	if (remove != NULL) {
