...
{
	//TODO: Your code here
	//Initializing pointer to use through all calls
	char* beginning_pointer = str;

	//Check if str is null, pointer should be at the saveptr position
	if (str == NULL) {
		beginning_pointer = *saveptr;
	}

	if (beginning_pointer == NULL) return NULL;
	str = beginning_pointer;
	int done = 0;
	while (*str != delim) {
		if (*str == '\0') {
			done = 1;
